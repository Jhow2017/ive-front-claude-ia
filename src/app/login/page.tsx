...
const TABS: { k: Role; l: string; placeholder: string }[] = [
  ...
  { k: "profissional", l: "Profissional", placeholder: "profissional@iverde.com.br" },
...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-(--color-bg) p-5 overflow-auto">
      <div className="bg-white border border-(--color-border-soft) rounded-2xl p-10 w-full max-w-[420px] shadow-md">
        <div className="flex items-center gap-2.5 mb-7 justify-center">
          <LogoMark size={36} />
          <div className="text-[15px] font-extrabold">
            Instituto <em className="text-(--color-g500) not-italic">Verde</em> Esperança
          </div>
        </div>
        <h1 className="text-[22px] font-extrabold tracking-tight mb-1">Acessar plataforma</h1>
        <p className="text-sm text-(--color-muted) mb-6">Entre com sua conta para continuar.</p>
          ...
          {TABS.map((t) => (
            <button
              ...
              className={`flex-1 py-2 text-[13px] font-semibold rounded-md transition-all ${
                tab === t.k ? "bg-white text-(--color-text) shadow-sm" : "text-(--color-muted)"
              }`}
            >
              {t.l}
            </button>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-3.5">
          <FormField label="E-mail">
            <Input type="email" placeholder={cur.placeholder} {...form.register("email")} />
        ...
        <div className="text-center mt-3.5 text-[13px] text-(--color-muted)">
          <button className="text-(--color-g500) font-semibold hover:underline">Esqueci minha senha</button>
          {" · "}
          <Link href="/" className="text-(--color-g500) font-semibold no-underline hover:underline">
            Voltar ao site
          </Link>
        </div>
      </div>
