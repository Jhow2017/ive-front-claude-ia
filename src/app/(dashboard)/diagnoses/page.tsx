...
const PENDENTES = [
    {
        ...
        data: "24/04/2026",
        espera: "2 dias",
        sintomas: "Dificuldade de concentração, hiperatividade intensa",
    },
    {
        prio: "Média",
        paciente: { seed: 2, name: "Pedro Santos" },
        tipo: "Comportamental",
        prof: "Dra. Mariana Souza",
        data: "22/04/2026",
        espera: "4 dias",
        sintomas: "Resistência a mudanças de rotina, irritabilidade",
    },
    {
        prio: "Baixa",
        paciente: { seed: 3, name: "Ana Costa Lima" },
        tipo: "Fonoaudiológico",
        prof: "Dra. Ana Lima",
        data: "20/04/2026",
        espera: "6 dias",
        sintomas: "Atraso no desenvolvimento da linguagem",
    },
];
const CONCLUIDOS = [
    {
        paciente: { seed: 1, name: "Maria Oliveira" },
        tipo: "Ocupacional",
        prof: "Profa. Cíntia",
        data: "15/04/2026",
        cid: "F84.0",
        desc: "Autismo Infantil — Integração sensorial prejudicada",
    },
    {
        paciente: { seed: 4, name: "Beatriz Almeida" },
        tipo: "Neurológico",
        prof: "Dr. Rafael Cunha",
        data: "10/04/2026",
        cid: "F90.0",
        desc: "Hiperatividade — Plano de intervenção definido",
    },
];
...
export default function DiagnosesView() {
        ...
        <>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                {[
                    ...
                    {
                        l: "Prioridade Alta",
                        v: PENDENTES.filter((p) => p.prio === "Alta").length,
                        c: "text-(--color-danger)",
                    },
                    {
                        l: "Prioridade Média",
                        v: PENDENTES.filter((p) => p.prio === "Média").length,
                        c: "text-(--color-warn)",
                    },
                    {
                        l: "Prioridade Baixa",
                        v: PENDENTES.filter((p) => p.prio === "Baixa").length,
                        c: "text-(--color-g500)",
                    },
                ].map((m, i) => (
                    <div
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-[64px_1fr] gap-4 p-5">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex gap-4 flex-wrap text-xs text-(--color-muted) mb-2">
                                                ...
                                                <div className="flex items-center gap-1">
                                                    <Calendar size={11} /> Solicitado em {d.data}
                                                </div>
                                                <div className="flex items-center gap-1 text-(--color-danger)">
                                                    <Clock size={11} color="var(--color-danger)" />{" "}
                                                    {d.espera} aguardando
                                                </div>
                                            </div>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-[1fr_auto] gap-4 p-5">
                                    <div>
                                        ...
                                        <div className="flex gap-4 flex-wrap text-xs text-(--color-muted) mb-2">
                                            ...
                                            <div className="flex items-center gap-1">
                                                <Calendar size={11} /> Concluído em {d.data}
                                            </div>
                                        </div>
                                        <div className="bg-(--color-bg) rounded-lg px-3 py-2.5 mt-2.5 text-[13px]">
                                            {d.desc}
                                        </div>
                                    </div>
