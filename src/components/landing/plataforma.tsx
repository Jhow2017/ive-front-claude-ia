import { BarChart3, Calendar, MessageSquare, FileText, ArrowRight, Bell, Settings } from "lucide-react";
import Link from "next/link";
import { Button } from "@/components/ui/button";

const FEATURES = [
  { Ic: BarChart3, t: "Evolução por área", d: "Relatórios mensais com metas e progressos por especialidade." },
  { Ic: Calendar, t: "Agenda integrada", d: "Sessões, confirmações e histórico centralizados." },
  { Ic: MessageSquare, t: "Canal com a equipe", d: "Fale diretamente com os profissionais, sem intermediários." },
  { Ic: FileText, t: "Prontuário acessível", d: "Diagnósticos, plano terapêutico e observações de sessão." },
];
        ...
        <div className="grid lg:grid-cols-[1fr_1.5fr] gap-20 items-center">
          <div>
            ...
            <p className="text-[15px] text-white/50 leading-relaxed mb-9">
              Uma área exclusiva para pais e responsáveis — onde o processo é visível, não apenas relatado uma vez por mês.
            </p>
            <div className="flex flex-col gap-4 mb-8">
              {FEATURES.map((f, i) => (
                <div key={i} className="flex gap-3.5 items-start">
                  ...
                  <div>
                    <h4 className="text-sm font-semibold text-white mb-0.5">{f.t}</h4>
                    <p className="text-[13px] text-white/45 leading-snug">{f.d}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-[#131f14] rounded-2xl border border-white/[0.07] overflow-hidden shadow-2xl">
            ...
            <div className="flex">
                    ...
                    {g.slice(1).map((item, ii) => (
                      <div
                        ...
                        className={`flex items-center gap-2 px-2 py-1.5 rounded-md mb-px ${
                          gi === 0 && ii === 0 ? "bg-(--color-g500)/15" : ""
                        }`}
                      >
                        <div
                          className={`w-1 h-1 rounded-full shrink-0 ${
                            gi === 0 && ii === 0 ? "bg-(--color-g500)" : "bg-white/20"
                          }`}
                        />
                        <span
                          className={`text-[11px] ${
                            gi === 0 && ii === 0 ? "text-white/85 font-medium" : "text-white/30"
                          }`}
                        >
                          {item}
                        </span>
              ...
              <div className="flex-1 p-4 overflow-hidden min-w-0">
                <div className="text-xs font-bold text-white/80 mb-3">Lucas Costa · 6 anos</div>
                <div className="grid grid-cols-3 gap-2 mb-3">
                  {[
                    ...
                    <div key={i} className="bg-[#1a2419] border border-white/5 rounded-lg p-3">
                      <div className="text-[9px] text-white/35 uppercase tracking-wide mb-0.5">{m.l}</div>
                      <div className="text-[15px] font-bold text-white">{m.v}</div>
                      <div className="text-[9px] text-(--color-g500) mt-px font-medium">{m.d}</div>
                    </div>
                  ))}
                </div>
                <div className="bg-[#1a2419] border border-white/5 rounded-lg p-3 mb-3">
                  <div className="text-[9px] text-white/35 uppercase tracking-wide mb-2.5">Frequência · 12 meses</div>
                  <svg viewBox={`0 0 ${W} ${H}`} className="w-full block" style={{ height: H }}>
                    <defs>
                      <linearGradient id="cg2" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%" stopColor="#259a43" stopOpacity=".25" />
                        <stop offset="100%" stopColor="#259a43" stopOpacity="0" />
                      </linearGradient>
                    </defs>
                    <path d={area} fill="url(#cg2)" />
                    <path d={line} fill="none" stroke="#259a43" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                    <circle cx={pts[11].x} cy={pts[11].y} r="3" fill="#259a43" />
                  </svg>
                </div>
                <div className="bg-[#1a2419] border border-white/5 rounded-lg p-3">
                  <div className="text-[9px] text-white/30 uppercase tracking-wide mb-2">Próximas sessões</div>
                  {[
                    { n: "Fonoaudiologia", t: "Ter 29/04 · 14h00", b: "Confirmada" },
                    { n: "Terapia Ocup.", t: "Qui 01/05 · 10h30", b: "Pendente" },
                  ].map((r, i) => (
                    <div
                      key={i}
                    ...
                    >
                      <div>
                        <div className="text-[11px] font-medium text-white/70">{r.n}</div>
                        <div className="text-[9px] text-white/25">{r.t}</div>
                      </div>
