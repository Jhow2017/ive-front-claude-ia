...
export function HeroCarousel() {
  ...
  const right = (r: Slide["right"]) => {
    ...
    if (r === "therapy") return <div className="flex justify-center items-end"><IllusTherapy /></div>;
    return <div className="flex justify-center items-end"><IllusBlocks /></div>;
  };
    ...
    >
                ...
                <div>
                  ...
                  >
                    <span
                      ...
                      style={{ background: isDark ? "rgba(255,255,255,.7)" : "var(--color-g500)" }}
                    />
                    {sl.chip}
                  </div>
                  <h1
                  ...
                  <div className="flex gap-3 flex-wrap">
                    ...
                    <Button
                      size="xl"
                      variant="outline"
                      className={isDark ? "bg-transparent !text-white !border-white/40" : ""}
                    >
                      Ver como funciona
                    </Button>
                  </div>
                  <div
                    className={`flex gap-8 mt-9 pt-9 border-t ${
                      isDark ? "border-white/20" : "border-(--color-border-soft)"
                    }`}
                  >
                    {STATS.map((st, j) => (
                      <div key={j}>
                        ...
                        <div
                          className={`text-[11px] mt-0.5 ${
                            isDark ? "text-white/65" : "text-(--color-muted)"
                          }`}
                        >
                          {st.l}
                        </div>
      ...
      <button
        ...
        className={`absolute top-1/2 -translate-y-1/2 left-6 w-11 h-11 rounded-full flex items-center justify-center cursor-pointer shadow-md z-10 hidden md:flex ${
          isDark ? "bg-white/15 border border-white/25 text-white" : "bg-white border border-(--color-border-soft) text-(--color-muted) hover:text-(--color-g500)"
        }`}
        aria-label="Anterior"
      >
        <ChevronLeft size={20} />
