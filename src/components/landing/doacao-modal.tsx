...
export function DoacaoModal() {
  ...
  const cc = useForm<CCForm>({ resolver: zodResolver(ccSchema), defaultValues: { num: "", nome: "", exp: "", cvv: "" } });
      ...
      <Dialog open onClose={close} size="sm">
        <div className="text-center py-5">
          ...
          <div className="text-[15px] text-(--color-muted) leading-relaxed mb-6">
            Sua doação foi registrada. Você receberá uma confirmação por e-mail em breve. Cada contribuição ajuda
            uma criança a ter acesso ao tratamento que precisa.
          </div>
  ...
  return (
    <Dialog open onClose={close} size="sm">
      <DialogHeader
        ...
        onClose={close}
      />
      <label className="text-xs font-semibold text-(--color-text) mb-1.5 block">Valor da doação</label>
        ...
        {[
          ...
          <button
            ...
            className={`flex-1 py-2 text-[13px] font-semibold rounded-lg flex items-center justify-center gap-1.5 transition-all ${
              tab === k ? "bg-white text-(--color-text) shadow-sm" : "text-(--color-muted)"
            }`}
          >
            <Ic size={14} /> {l}
          </button>
      ...
      {tab === "pix" && (
        <div className="text-center">
          <div className="w-36 h-36 rounded-2xl bg-(--color-bg) border-2 border-dashed border-(--color-border-soft) flex flex-col items-center justify-center mx-auto mb-4 gap-2">
            ...
            <span className="text-[10px] text-(--color-muted) font-semibold">QR Code PIX</span>
          </div>
          ...
          <div className="bg-(--color-bg) border border-(--color-border-soft) rounded-lg px-3.5 py-2.5 flex justify-between items-center mb-5">
            <span className="text-xs text-(--color-muted) font-mono break-all leading-snug">{PIX_KEY}</span>
            <button
              type="button"
              onClick={() => {
          ...
          <p className="text-[11px] text-(--color-muted) leading-relaxed mb-4">
            Após o pagamento, envie o comprovante para <strong>doacoes@iverde.org.br</strong>.
          </p>
      ...
      {tab === "cc" && (
        <form onSubmit={cc.handleSubmit(onCc)} className="grid grid-cols-2 gap-3">
          <FormField label="Número do cartão" className="col-span-2">
            <Input placeholder="0000 0000 0000 0000" maxLength={19} {...cc.register("num")} />
            {cc.formState.errors.num && (
              <span className="text-[11px] text-(--color-danger) mt-1">{cc.formState.errors.num.message}</span>
            )}
          </FormField>
