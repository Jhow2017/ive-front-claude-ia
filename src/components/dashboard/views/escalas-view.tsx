...
import { Plus, ChevronLeft, ChevronRight, ChevronDown, Calendar, Clock, Users, Check } from "lucide-react";
...
const PROFS = [
  {
    seed: 0, name: "Dra. Ana Lima", spec: "Fonoaudiologia",
    schedule: [["08:00-12:00", "14:00-18:00"], ["08:00-12:00", "14:00-18:00"], ["08:00-12:00"], [null, "14:00-18:00"], ["08:00-12:00", "14:00-18:00"], [null]],
    turnos: 9, dias: 5,
  },
  {
    seed: 1, name: "Dr. Rafael Cunha", spec: "Neuropsicologia",
    schedule: [["09:00-13:00"], ["09:00-13:00"], ["09:00-13:00", "15:00-19:00"], ["09:00-13:00"], ["09:00-13:00"], [null]],
    turnos: 6, dias: 5,
  },
  {
    seed: 2, name: "Profa. Cíntia", spec: "Terapia Ocupacional",
    schedule: [["08:00-17:00"], [null], ["08:00-17:00"], [null], ["08:00-17:00"], ["08:00-12:00"]],
    turnos: 4, dias: 4,
  },
];
  ...
  return (
    <>
      <div className="flex justify-between items-start flex-wrap gap-3">
        <div>
          <div className="text-[22px] font-extrabold tracking-tight">Escalas de Atendimento</div>
          <div className="text-[13px] text-(--color-muted) mt-1">Horários e disponibilidade da equipe</div>
        </div>
        <Button>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <div className="flex items-center gap-2.5 flex-1">
              ...
              <div>
                ...
                <div className="text-xs text-(--color-muted)">Escalas e horários de atendimento</div>
              </div>
            </div>
            <div className="flex gap-2 items-center relative">
              ...
              <div className="relative">
                <Button variant="outline" size="sm" onClick={() => setPeriodOpen((o) => !o)}>
                  {period === "hoje" ? "Hoje" : period === "mes" ? "Este Mês" : "Esta Semana"}
                  <ChevronDown size={12} />
                </Button>
                {periodOpen && (
                  <div
                  ...
                  >
                    {[
                      ...
                      <button
                        key={k}
                        onClick={() => { setPeriod(k as any); setWeekOffset(0); setPeriodOpen(false); }}
                        className={`flex items-center justify-between w-full px-2.5 py-2 rounded-md text-[13px] font-medium hover:bg-(--color-g50) ${
                          period === k ? "bg-(--color-g50) text-(--color-g600) font-semibold" : ""
                        }`}
                      >
                        {l}
                        {period === k && <Check size={13} color="var(--color-g500)" />}
                      </button>
                    ))}
                  </div>
                )}
      ...
      {PROFS.map((p, pi) => (
        <div key={pi} className="bg-white border border-(--color-border-soft) rounded-xl mb-3.5 overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-(--color-border-soft) gap-3">
            <div className="flex items-center gap-3 min-w-0 flex-1 overflow-hidden">
              ...
              <div className="min-w-0">
                ...
                <div className="text-xs text-(--color-muted) truncate">{p.spec}</div>
              </div>
            </div>
            <Button variant="outline" size="sm">Editar Escala</Button>
          </div>
