...
const ALL: Ev[] = [
    {
        col: 0,
        row: 1,
        label: "Fono · Lucas Costa",
        cls: "ev-green",
        spec: "Fonoaudiologia",
        pac: { seed: 0, name: "Lucas Costa" },
    ...
    },
    {
        col: 1,
        row: 3,
        label: "TO · Maria Oliveira",
        cls: "ev-blue",
        spec: "Terapia Ocupacional",
        pac: { seed: 2, name: "Maria Oliveira" },
    ...
    },
    {
        col: 2,
        row: 0,
        label: "Psico · Pedro Santos",
        cls: "ev-purple",
        spec: "Psicologia",
        pac: { seed: 3, name: "Pedro Santos" },
    ...
    },
    {
        col: 3,
        row: 5,
        label: "Neuro · Ana Lima",
        cls: "ev-yellow",
        spec: "Neuropsicologia",
        pac: { seed: 4, name: "Ana Lima" },
    ...
    },
    {
        col: 4,
        row: 2,
        label: "Fono · Carlos Rocha",
        cls: "ev-green",
        spec: "Fonoaudiologia",
        pac: { seed: 5, name: "Carlos Rocha" },
    ...
    },
    {
        col: 0,
        row: 6,
        label: "Fono · Beatriz A.",
        cls: "ev-green",
        spec: "Fonoaudiologia",
        pac: { seed: 1, name: "Beatriz Alves" },
        prof: { seed: 0, name: "Dra. Renata Lima", id: "renata" },
        status: "Confirmada",
        sala: "Sala 3",
        familia: "alves",
    ...
    return (
        <>
            ...
            {view === "semana" && (
                <div className="grid grid-cols-[52px_repeat(5,1fr)] rounded-xl overflow-hidden border border-(--color-border-soft) bg-white">
                    ...
                    {DAYS.map((d, i) => (
                        <div
                            key={i}
                            className="bg-(--color-bg) py-2.5 text-center border-b border-(--color-border-soft)"
                        >
                            <div className="text-[10px] font-semibold text-(--color-muted) uppercase tracking-wide">
                                {d.d}
                            </div>
                            <div
                                            ...
                                            >
                                                {isProf ? (
                                                    <>
                                                        ...
                                                        <div className="text-[9px] opacity-75 mt-px">
                                                            {ev.sala}
                                                        </div>
                                                    </>
                                                ) : (
                            ...
                            return (
                                <Fragment key={ti}>
                                    ...
                                    <div className="border-t border-(--color-border-soft) min-h-16 px-3 py-1.5 flex flex-col gap-1 hover:bg-(--color-g50)">
                                        {dayEvents.length === 0 && (
                                            <div className="text-[11px] text-(--color-muted2) italic">
                                                Disponível
                                            </div>
                                        )}
                                        {dayEvents.map((ev, ei) => (
            ...
            <Dialog open={modalOpen} onClose={() => setModalOpen(false)}>
                <DialogHeader
                    title="Nova Consulta"
                    sub="Agendar nova sessão de atendimento"
                ...
                <div className="grid grid-cols-2 gap-3">
                    ...
                    <FormField label="Profissional *">
                        <Select>
                            ...
                            <option>Dr. Rafael Cunha</option>
                            <option>Profa. Cíntia</option>
                        </Select>
                    </FormField>
                        ...
                        <div className="flex flex-col gap-3.5">
                            ...
                            <div className="grid grid-cols-2 gap-2.5">
                                <div className="bg-white border border-(--color-border-soft) rounded-xl p-4">
                                    <div className="text-[11px] text-(--color-muted) uppercase font-semibold">
                                        Status
                                    </div>
                                    <div
...
function MonthView({ events, setSel }: { events: Ev[]; setSel: (e: any) => void }) {
    const mDays = Array.from({ length: 35 }, (_, i) => {
        ...
        const evs = inMonth
            ? events.filter((e) => (e.col + e.row) % 12 === num % 12).slice(0, 3)
            : [];
        return {
                    ...
                    {d.evs.map((ev, ei) => (
                        <div
                            ...
                            onClick={() =>
                                setSel({
                                    ...ev,
                                    day: { d: "Qua", dt: String(d.num).padStart(2, "0") },
                                    time: TIMES[ev.row],
                                })
                            }
