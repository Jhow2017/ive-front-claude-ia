...
export function PainelView() {
  ...
  const tt = time ? time.toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" }) : "--:--";
  const dd = time ? time.toLocaleDateString("pt-BR", { weekday: "long", day: "2-digit", month: "long" }) : "";
    ...
    <>
      {!fullscreen && (
        <div className="flex justify-between items-start flex-wrap gap-3">
          <div>
            <div className="text-[22px] font-extrabold tracking-tight">Painel da TV</div>
      ...
      >
        ...
        <div className="flex justify-between items-center mb-8 relative">
          <div className="flex items-center gap-3.5">
            <LogoMark size={48} />
            <div>
              <div className="text-[22px] font-extrabold tracking-tight">Instituto Verde Esperança</div>
              <div className="text-[13px] text-[#9fc4ad] capitalize">{dd}</div>
            </div>
          </div>
          <div className="text-5xl font-extrabold tabular-nums text-(--color-g200)">{tt}</div>
        </div>

        <div className="bg-gradient-to-br from-(--color-g600) to-(--color-g500) rounded-3xl px-12 py-10 mb-6 relative shadow-2xl">
          <div className="text-sm font-bold tracking-[2px] uppercase text-white/85 mb-2">Chamando agora</div>
          {chamando ? (
            <>
              <div
              ...
              <div className="flex gap-8 text-xl font-semibold text-white/95 flex-wrap">
                <div>
                  <span className="opacity-70 text-sm uppercase tracking-widest block">Sala</span>
                  {chamando.sala}
                </div>
                <div>
                  <span className="opacity-70 text-sm uppercase tracking-widest block">Profissional</span>
                  {chamando.prof}
                </div>
                <div>
                  <span className="opacity-70 text-sm uppercase tracking-widest block">Especialidade</span>
                  {chamando.spec}
                </div>
              </div>
            </>
          ) : (
            <div className="text-5xl font-bold opacity-60 py-5">Aguardando próxima chamada…</div>
          )}
        </div>
            ...
            <div className="grid gap-2.5">
              {aguardando.slice(0, 5).map((p, i) => (
                ...
                >
                  ...
                  <div className="text-sm text-[#9fc4ad]">{p.sala} · {p.prof}</div>
                </div>
              ))}
              {aguardando.length === 0 && (
                <div className="text-[#9fc4ad] py-6 text-center">Sem pacientes aguardando</div>
              )}
            </div>
          </div>
          <div>
