...
interface Consulta {
  day: string; mon: string; time: string;
...
const CONSULTAS: Consulta[] = [
  { day: "29", mon: "ABR", time: "14:00", title: "Sessão de Fonoaudiologia", type: "Rotina", paciente: { seed: 0, name: "Lucas Costa" }, prof: { seed: 1, name: "Dra. Ana Lima" }, status: "Confirmada", obs: "Boas respostas ao protocolo de nomeação. Exercício para casa enviado." },
  { day: "28", mon: "ABR", time: "10:30", title: "Terapia Ocupacional", type: "Rotina", paciente: { seed: 2, name: "Maria Oliveira" }, prof: { seed: 2, name: "Profa. Cíntia" }, status: "Concluída", obs: "Trabalho de integração sensorial. Boa tolerância ao toque." },
  { day: "25", mon: "ABR", time: "09:00", title: "Avaliação Neuropsicológica", type: "Avaliação", paciente: { seed: 3, name: "Pedro Santos" }, prof: { seed: 3, name: "Dr. Rafael Cunha" }, status: "Concluída", obs: "Reavaliação cognitiva — relatório gerado." },
  { day: "22", mon: "ABR", time: "15:00", title: "Sessão de Psicologia", type: "Retorno", paciente: { seed: 4, name: "Ana Costa Lima" }, prof: { seed: 4, name: "Dra. Mariana Souza" }, status: "Faltou", obs: "Paciente não compareceu. Contato realizado pela equipe." },
  { day: "20", mon: "ABR", time: "11:00", title: "Terapia Ocupacional", type: "Rotina", paciente: { seed: 0, name: "Lucas Costa" }, prof: { seed: 2, name: "Profa. Cíntia" }, status: "Cancelada", obs: "Cancelado pela família. Reagendamento realizado." },
];
  ...
  return (
    <>
      <div className="flex justify-between items-start flex-wrap gap-3">
        <div>
          <div className="text-[22px] font-extrabold tracking-tight">Histórico de Consultas</div>
          <div className="text-[13px] text-(--color-muted) mt-1">Consultas realizadas, canceladas e faltas</div>
        </div>
            ...
            {(["hoje", "semana", "mes"] as const).map((k) => (
              <button
                ...
                className={`px-3.5 py-1.5 text-[13px] font-medium rounded-md transition-all ${
                  period === k ? "bg-white text-(--color-text) font-semibold shadow-sm" : "text-(--color-muted)"
                }`}
              >
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <div className="flex items-center gap-2 flex-1">
              ...
              <div>
                ...
                <div className="text-xs text-(--color-muted)">{stats.total} consultas encontradas</div>
              </div>
            </div>
          </div>
        </CardContent>
        ...
        >
          <div className="grid grid-cols-1 sm:grid-cols-[72px_1fr] gap-4 p-5">
            <div className="bg-(--color-g50) rounded-[10px] p-2.5 text-center shrink-0 w-fit">
              <div className="text-[22px] font-extrabold text-(--color-g600) leading-none">{c.day}</div>
          ...
          <div className="border-t border-(--color-border-soft) px-5 py-3 flex justify-end gap-2">
            <Button variant="outline" size="sm" onClick={() => setSel(c)}>Ver Detalhes</Button>
      ...
      <Dialog open={!!sel} onClose={() => setSel(null)}>
        {sel && (
          <>
            <DialogHeader title={sel.title} sub={`${sel.day} ${sel.mon} · ${sel.time}`} onClose={() => setSel(null)} />
            <div className="flex flex-col gap-3">
              ...
              <div>
                <div className="text-[11px] text-(--color-muted) mb-1.5 font-semibold">OBSERVAÇÕES</div>
                <div className="text-[13px] bg-(--color-bg) rounded-lg p-3 leading-relaxed">{sel.obs}</div>
              </div>
            </div>
            <DialogFooter>
