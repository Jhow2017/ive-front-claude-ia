...
import { ChevronDown, Plus, X, Check, Trash2, User, Users, Upload, FileText, Phone, AlertCircle } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input, Select, FormField } from "@/components/ui/input";

const ESTADOS = ["SP", "RJ", "MG", "RS", "PR", "SC", "BA", "CE", "GO", "DF"];
const DOCS = ["RG (frente)", "CPF (frente)", "Comprovante de Nascimento", "Cartão SUS", "Histórico Médico", "Laudo (se houver)"];
  ...
  return (
    <div>
      ...
      >
        ...
        <div className="text-[11px] text-(--color-muted2) mt-0.5">PDF, JPG, PNG · até 10MB</div>
        <input
          ref={inputRef}
          type="file"
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
        <FormField label="Nome Completo *" className="sm:col-span-2"><Input placeholder="Digite o nome completo" /></FormField>
        ...
        <FormField label="Cartão SUS"><Input placeholder="000 0000 0000 0000" /></FormField>
        <FormField label="NIS (Bolsa Família)"><Input placeholder="000.00000.00-0" /></FormField>
      </div>
      <SectionLabel Ic={Phone}>Contato</SectionLabel>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
        <FormField label="Telefone *"><Input placeholder="(000) 00000-0000" /></FormField>
        <FormField label="Email"><Input type="email" placeholder="email@exemplo.com" /></FormField>
        <FormField label="Contato de Emergência"><Input placeholder="Nome do contato" /></FormField>
        <FormField label="Telefone de Emergência"><Input placeholder="(011) 00000-0000" /></FormField>
      </div>
      <SectionLabel Ic={FileText}>Endereço</SectionLabel>
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 mb-3">
        <FormField label="CEP"><Input placeholder="00000-000" /></FormField>
        <FormField label="Logradouro" className="sm:col-span-2"><Input placeholder="Rua, Avenida..." /></FormField>
        <FormField label="Número"><Input placeholder="Nº" /></FormField>
        <FormField label="Complemento"><Input placeholder="Apto, Bloco..." /></FormField>
        <FormField label="Bairro"><Input /></FormField>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
        {DOCS.map((d, i) => <DocUpload key={i} label={d} />)}
      </div>
    </>
  );
}
    ...
    >
      ...
      >
        <div className="flex items-center gap-3">
          ...
          >
            <Ic size={18} color={dependente ? "var(--color-purple)" : "var(--color-g500)"} />
          </div>
          <div>
      ...
      >
        <div className="overflow-hidden">
          <div className="px-5 py-5 border-t border-(--color-border-soft)">{children}</div>
        </div>
      </div>
    </div>
  );
...
export function CadastrarPacienteView() {
      ...
      <div className="flex justify-between items-start flex-wrap gap-3">
        <div>
          ...
          <div className="text-[13px] text-(--color-muted) mt-1">
            Preencha os dados para cadastrar um <span className="text-(--color-g500) font-semibold">novo paciente</span>
          </div>
        </div>
      </div>
