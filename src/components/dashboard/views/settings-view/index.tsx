...
function NotifChannel({
    ...
    accent: string;
    st: NotifState;
    set: (s: NotifState) => void;
}) {
    return (
        <Card
            style={{
                borderColor: st.on ? accent : "var(--color-border-soft)",
                borderWidth: st.on ? 2 : 1,
            }}
        >
            <CardContent>
                ...
                {st.on && (
                    <>
                        ...
                        <div className="flex gap-2 flex-wrap mb-3.5">
                            {[
                                ...
                                <label
                                    ...
                                    style={{
                                        background: st.when.includes(k)
                                            ? "var(--color-g50)"
                                            : "white",
                                        borderColor: st.when.includes(k)
                                            ? "var(--color-g500)"
                                ...
                                >
                                    <input
                                        ...
                                        onChange={() =>
                                            set({
                                                ...st,
                                                when: st.when.includes(k)
                                                    ? st.when.filter((x) => x !== k)
                                                    : [...st.when, k],
                                            })
                                        }
                                    />
                        ...
                        <div className="mt-3.5 p-3.5 bg-(--color-bg) rounded-[10px]">
                            <div className="text-[11px] font-bold text-(--color-muted) mb-2">
                                PRÉ-VISUALIZAÇÃO
                            </div>
