...
import { Plus, Search, Filter, FileText, Edit, Trash2, MoreHorizontal, Phone, Mail, Calendar as CalIco, X, Check } from "lucide-react";
...
const PACIENTES: Paciente[] = [
  { id: 1, seed: 0, name: "Lucas Costa", email: "lucas@email.com", cpf: "123.456.789-00", phone: "(11) 98765-4321", lastVisit: "29 Abr 2026", status: "Ativo" },
  { id: 2, seed: 1, name: "Maria Oliveira", email: "maria.oliveira@email.com", cpf: "234.567.890-11", phone: "(11) 98765-1234", lastVisit: "25 Abr 2026", status: "Ativo" },
  { id: 3, seed: 2, name: "Pedro Santos", email: "pedro.santos@email.com", cpf: "345.678.901-22", phone: "(11) 98765-5678", lastVisit: "22 Abr 2026", status: "Ativo" },
  { id: 4, seed: 3, name: "Ana Costa Lima", email: "ana.lima@email.com", cpf: "456.789.012-33", phone: "(11) 98765-9012", lastVisit: "18 Abr 2026", status: "Ativo" },
  { id: 5, seed: 4, name: "Beatriz Almeida", email: "beatriz@email.com", cpf: "567.890.123-44", phone: "(11) 98765-3456", lastVisit: "10 Abr 2026", status: "Inativo" },
];
...
export function PacientesView() {
  ...
  const filtered = PACIENTES.filter((p) => {
    ...
    const sm = !search || p.name.toLowerCase().includes(search.toLowerCase()) || p.cpf.includes(search) || p.email.includes(search);
    return fm && sm;
  });
    ...
    <>
      <div className="flex justify-between items-start flex-wrap gap-3">
        <div>
          ...
          <div className="text-[13px] text-(--color-muted) mt-1">Gerenciar cadastros e atendimentos</div>
        </div>
        ...
        <CardContent>
          ...
          <div className="flex gap-1.5 mb-4 flex-wrap">
            {(["todos", "ativos", "inativos"] as const).map((k) => (
              ...
              >
                {k === "todos" ? "Todos" : k === "ativos" ? "Ativos" : "Inativos"} ({counts[k]})
              </button>
            ))}
          </div>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr>
                  {["Paciente", "CPF", "Contato", "Última Visita", "Status", ""].map((h, i) => (
                    <th
                      key={i}
              ...
              <tbody>
                {filtered.map((p) => (
                  <tr key={p.id} className="hover:bg-(--color-g50)">
                    <td className="py-3 px-3.5 border-t border-(--color-border-soft)">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          ...
                          <div className="text-[11px] text-(--color-muted)">{p.email}</div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="py-3 px-3.5 border-t border-(--color-border-soft)">
                      <div className="flex flex-col gap-0.5">
                        <div className="text-xs flex items-center gap-1.5 text-(--color-muted)">
                          <Phone size={11} color="var(--color-g500)" /> {p.phone}
                        </div>
                    ...
                    <td className="py-3 px-3.5 border-t border-(--color-border-soft)">
                      <Badge variant={p.status === "Ativo" ? "green" : "gray"}>{p.status}</Badge>
                    </td>
                    <td className="py-3 px-3.5 border-t border-(--color-border-soft)">
                      <div className="flex gap-1 items-center justify-end">
                        <Button size="icon" variant="ghost" title="Ver" onClick={() => setView(p)}>
                          <FileText size={14} />
                        </Button>
                        <Button size="icon" variant="ghost" title="Editar" onClick={() => setEdit(p)}>
                          <Edit size={14} />
                        </Button>
                        <Button size="icon" variant="danger" title="Excluir" onClick={() => setDel(p)}>
                          <Trash2 size={14} />
                        </Button>
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-2.5 mb-3.5">
              <div className="bg-white border border-(--color-border-soft) rounded-xl p-4">
                <div className="text-[11px] font-semibold text-(--color-muted) uppercase mb-1.5">Última visita</div>
                <div className="text-sm font-bold">{view.lastVisit}</div>
              </div>
              <div className="bg-white border border-(--color-border-soft) rounded-xl p-4">
                <div className="text-[11px] font-semibold text-(--color-muted) uppercase mb-1.5">Status</div>
                <div className="text-sm font-bold text-(--color-g500)">{view.status}</div>
              </div>
            </div>
      ...
      <Dialog open={!!edit} onClose={() => setEdit(null)}>
        {edit && (
          <>
            <DialogHeader title="Editar paciente" sub={edit.name} onClose={() => setEdit(null)} />
            <div className="grid grid-cols-2 gap-3">
              <FormField label="Nome completo" className="col-span-2">
                <Input defaultValue={edit.name} />
              </FormField>
              <FormField label="CPF"><Input defaultValue={edit.cpf} /></FormField>
              <FormField label="Telefone"><Input defaultValue={edit.phone} /></FormField>
              <FormField label="E-mail" className="col-span-2"><Input defaultValue={edit.email} /></FormField>
            </div>
            <DialogFooter>
      ...
      <Dialog open={!!del} onClose={() => setDel(null)} size="sm">
        {del && (
          <>
            <DialogHeader title="Excluir paciente" sub="Esta ação não pode ser desfeita" onClose={() => setDel(null)} />
            <div className="text-sm py-1">
              Tem certeza que deseja excluir <b>{del.name}</b>? Os dados clínicos serão arquivados conforme a LGPD.
            </div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setDel(null)}>Cancelar</Button>
              <Button className="bg-(--color-danger) hover:bg-red-600" onClick={() => setDel(null)}>
