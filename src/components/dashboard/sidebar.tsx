...
export function Sidebar() {
  ...
  return (
    <>
      {mobileOpen && (
        <div className="fixed inset-0 bg-black/40 z-[99] backdrop-blur-sm md:hidden" onClick={() => setMobileOpen(false)} />
      )}
      <aside
        className={cn(
        ...
        <div className="px-3.5 py-3.5 flex items-center gap-2.5 border-b border-(--color-border-soft) shrink-0 h-[57px]">
          ...
          {!collapsed && (
            <div className="overflow-hidden whitespace-nowrap">
              ...
              <div className="text-[10px] text-(--color-muted) leading-tight">Esperança</div>
            </div>
          )}
        </div>
            ...
            <div key={gi}>
              ...
              {grp.items.map((item) => {
                  ...
                  >
                    <div
                      className={cn(
                        ...
                        isActive ? "bg-(--color-g500) text-white" : "text-(--color-muted)"
                      )}
                    >
                      <NavIcon name={item.icon} size={15} color={isActive ? "#fff" : undefined} />
                    </div>
                    {!collapsed && (
                      <>
                        <span
                          className={cn(
                            ...
                            isActive ? "text-(--color-text) font-semibold" : "text-(--color-muted) font-medium"
                          )}
                        >
                          {item.label}
                        </span>
                        ...
                        {hasChildren && (
                          <ChevronDown
                            size={13}
                            className={cn("shrink-0 text-(--color-muted2) transition-transform", isOpen && "rotate-180")}
                          />
                        )}
                      </>
                    )}
                ...
                return (
                  <div key={item.id}>
                    {hasChildren ? (
                      <button onClick={() => toggle(item.id)} className="w-full text-left">
                        {content}
                      </button>
                    ) : (
                          ...
                          return (
                            <Link
                              ...
                              className={cn(
                                ...
                                subActive ? "bg-(--color-g50)" : "hover:bg-(--color-g50)"
                              )}
                            >
                              <div
                                className={cn(
                                  "w-1.5 h-1.5 rounded-full shrink-0",
                                  subActive ? "bg-(--color-g500)" : "bg-(--color-border-soft)"
                                )}
                              />
                              <span
                                className={cn(
                                  "text-xs",
                                  subActive ? "text-(--color-text) font-semibold" : "text-(--color-muted) font-medium"
                                )}
                              >
                                {c.label}
                              </span>
